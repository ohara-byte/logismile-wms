/**
 * PUT    /api/master/noshi-exclusions/[id]   更新
 * DELETE /api/master/noshi-exclusions/[id]   削除
 */
...
export async function PUT(
  ...
  try {
    const updated = await prisma.noshiExclusion.update({ where: { id }, data: parsed.data });
...
export async function DELETE(
  ...
  try {
    await prisma.noshiExclusion.delete({ where: { id } });
    return NextResponse.json({ data: { id }, message: 'OK' });
  } catch (e) {
    return maskError(
      '[DELETE /api/master/noshi-exclusions]',
      e,
      'NOT_FOUND',
      404,
      '削除できません（対象が見つかりません）',
