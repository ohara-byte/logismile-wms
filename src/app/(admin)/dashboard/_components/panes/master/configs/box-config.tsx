...
/** 名称（太字）+ コード（モノ系小フォント）を 2 段表示。名称が見切れ難くなる */
function renderBoxNameCell(r: Record<string, unknown>): ReactNode {
  const name = String(r.name ?? '');
  const code = String(r.code ?? '');
  return (
    <div className="leading-tight">
      <div className="font-bold text-ink-strong truncate">{name || '—'}</div>
      <div className="font-mono text-3xs text-ink-muted truncate">{code}</div>
    </div>
  );
}
...
export const boxConfig: MasterConfig<Box> = {
  ...
  columns: [
    // Sprint Y-4: 名称＋コードを 2 段組セルに集約。コード単独の列は撤去
    {
      key: 'name',
      label: '名称 / コード',
      width: 200,
      truncate: true,
      render: (r) => renderBoxNameCell(r as Record<string, unknown>),
    },
    {
      key: 'type',
      label: '種別',
